...
### Research Question 1
Here, we addressed possible relationships between Need for Cognition (NFC), self-control, adaptive and maladaptive coping strategies, the emotion regulation strategies reappraisal and suppression, as well as the burnout dimension *reduced personal efficacy*. We observed `r classify_r(df_analysis$nfc, df_analysis$scs)` correlation between NFC and self-control (H1a), `r report_cor(df_analysis$nfc, df_analysis$scs)`. Concerning habitually used emotion regulation strategies (H1b), there was no relation with NFC, neither for *reappraisal*, `r report_cor(df_analysis$nfc, df_analysis$erq_reap)`, nor for *suppression*, `r report_cor(df_analysis$nfc, df_analysis$erq_supp)`. Regarding coping (H1c), there was `r classify_r(df_analysis$nfc, df_analysis$sci_a)` relation with *adaptive coping*, `r report_cor(df_analysis$nfc, df_analysis$sci_a)`, and `r classify_r(df_analysis$nfc, df_analysis$sci_ma)` correlation with *maladaptive coping*, `r report_cor(df_analysis$nfc, df_analysis$sci_ma)`. The burnout dimension *reduced personal efficiency* showed `r classify_r(df_analysis$nfc, df_analysis$mbi_rpe)` relationship with NFC, `r report_cor(df_analysis$nfc, df_analysis$mbi_rpe)` (H1d), `r classify_r(df_analysis$scs, df_analysis$mbi_rpe)` relationship with self-control, `r report_cor(df_analysis$scs, df_analysis$mbi_rpe)` (H1e), `r classify_r(df_analysis$erq_reap, df_analysis$mbi_rpe)` relationship with the habitual use of the emotion regulation strategy *reappraisal*, `r report_cor(df_analysis$erq_reap, df_analysis$mbi_rpe)` and `r classify_r(df_analysis$erq_supp, df_analysis$mbi_rpe)` relationship with *suppression*, `r report_cor(df_analysis$erq_supp, df_analysis$mbi_rpe)` (H1f). Finally, there was `r classify_r(df_analysis$sci_a, df_analysis$mbi_rpe)` correlation with *adaptive coping*, `r report_cor(df_analysis$sci_a, df_analysis$mbi_rpe)` and `r classify_r(df_analysis$sci_ma, df_analysis$mbi_rpe)` correlation with *maladaptive coping*, `r report_cor(df_analysis$sci_ma, df_analysis$mbi_rpe)` (H1g).

### Research Question 2
Concerning the burnout dimension *emotional exhaustion*, we observed correlations with all variables of interest, i.e., NFC, `r report_cor(df_analysis$nfc, df_analysis$mbi_ee)`, self-control, `r report_cor(df_analysis$scs, df_analysis$mbi_rpe)`, adaptive and maladaptive coping strategies, `r report_cor(df_analysis$sci_a, df_analysis$mbi_rpe)` and `r report_cor(df_analysis$sci_ma, df_analysis$mbi_rpe)`, as well as with the emotion regulation strategies reappraisal and suppression, `r report_cor(df_analysis$erq_reap, df_analysis$mbi_rpe)` and `r report_cor(df_analysis$erq_supp, df_analysis$mbi_rpe)`.

### Research Question 3
Regarding the burnout dimension *depersonalization*, no significant correlations were found for NFC, `r report_cor(df_analysis$nfc, df_analysis$mbi_de)`, and self-control, `r report_cor(df_analysis$scs, df_analysis$mbi_de)`, while there were significant relationships with adaptive and maladaptive coping strategies, `r report_cor(df_analysis$sci_a, df_analysis$mbi_de)` and `r report_cor(df_analysis$sci_ma, df_analysis$mbi_de)`, as well as with reappraisal and suppression, `r report_cor(df_analysis$erq_reap, df_analysis$mbi_de)` and `r report_cor(df_analysis$erq_supp, df_analysis$mbi_de)`. 
...
model_rpe = "
  ...
  # mediators
    ...
    sci_adapt ~ a4*nfc
    sci_maladapt ~ a5*nfc
    mbi_rpe ~ b1*scs + b2*erq_reap + b3*erq_supp + b4*sci_adapt + b5*sci_maladapt
  # indirect effects 
    ind1 := a1*b1
    ind2 := a2*b2
    ind3 := a3*b3
...
#summary(fit_rpe, standardized = T, ci = T)

report_pe <-  function(fit, label=NULL, txt = F) {
...
Figure 1 provides the results of our research questions regarding possible mediation effects  of the relation between NFC and the burnout dimensions reduced *personal efficiency* (Figure 1A), *emotional exhaustion* (Figure 1B), and *depersonalization* (Figure 1C). <!-- or three separate figures? -->

The total effect was `r report_pe(fit_rpe, c('c'), txt=T)`.
